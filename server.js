...
function imageCandidates($,base,vehicleNodes=[]){let out=[],seen=new Set();const add=u=>{u=abs(base,String(u||'').trim());if(!/^https:\/\//i.test(u)||seen.has(u))return;let low=u.toLowerCase();if(/logo|favicon|icon|sprite|avatar|tracking|pixel/i.test(low))return;if(!(/\.(jpg|jpeg|png|webp|avif)(\?|$)/i.test(u)||/image|photo|vehicle|media|inventory|cdn|dealerimage/i.test(low)))return;seen.add(u);out.push(u)};
  ...
  $('img,source').each((_,e)=>['data-photo','data-image','data-original','data-flickity-lazyload','data-lazy','data-src','data-lazy-src','data-url','src','srcset','data-srcset'].forEach(k=>{let val=$(e).attr(k);if(k.includes('srcset'))String(val||'').split(',').forEach(x=>add(x.trim().split(/\s+/)[0]));else add(val)}));
  $('[style*=background-image],[data-images],[data-photos]').each((_,e)=>{let raw=$(e).attr('style')||$(e).attr('data-images')||$(e).attr('data-photos')||'';String(raw).match(/https?:[^\"' )]+/g)?.forEach(add)});
...
function event(req,b){try{let d=ensure(read()),type=String(b.type||'').slice(0,40);if(!['page_view','vehicle_view','outbound_click','search','filter','dealer_view','install_prompt','push_subscribe'].includes(type))return;let v=d.vehicles.find(x=>x.id===b.vehicleId),dealer=d.dealers.find(x=>x.id===b.dealerId);d.analytics.events.push({ts:new Date().toISOString(),type,vehicleId:String(b.vehicleId||'').slice(0,80),vehicleName:String(b.vehicleName||v?.title||[v?.year,v?.make,v?.model].filter(Boolean).join(' ')||'').slice(0,200),dealerId:String(b.dealerId||'').slice(0,80),dealerName:String(b.dealerName||dealer?.name||'').slice(0,200),page:String(b.page||'').slice(0,120),filter:String(b.filter||'').slice(0,1000),session:hash(b.sessionId||crypto.randomUUID()),referrer:String(b.referrer||'').slice(0,250)});if(d.analytics.events.length>100000)d.analytics.events=d.analytics.events.slice(-100000);write(d);schedulePersist()}catch{}}
...
app.get('/health',(req,res)=>res.json({ok:true,service:'official-cars-api',version:'8.0.0',sync:syncState,persistence:{github:!!(GH.token&&GH.owner&&GH.repo),file:GH.file},push:{configured:!!(PUSH.publicKey&&PUSH.privateKey&&webpush)}}));
...
app.post('/api/admin/sync-dealer',auth,(req,res)=>{let d=ensure(read()),dealer=d.dealers.find(x=>x.id===req.body?.dealerId);if(!dealer)return res.status(404).json({error:'Dealer not found'});if(syncState.running)return res.status(409).json({error:'Another sync is already running'});startSyncState(dealer);syncDealer(dealer).then(n=>finishSync(n)).catch(e=>{syncState.errors.push(e.message);syncState.phase='failed';syncState.running=false;syncState.finishedAt=new Date().toISOString();log('FATAL '+e.message);schedulePersist()});res.json({ok:true,dealerId:dealer.id})});
function startSyncState(dealer){syncState={running:true,startedAt:new Date().toISOString(),finishedAt:null,found:0,imported:0,skipped:0,errors:[],log:[],currentDealer:dealer.id,currentUrl:null,totalUrls:0,processed:0,phase:'starting'}}
...
async function syncAll(){startSyncState({id:'all'});try{let d=ensure(read());for(const dealer of d.dealers.filter(x=>x.enabled!==false)){syncState.currentDealer=dealer.id;let n=await syncDealer(dealer);syncState.imported+=n}syncState.running=false;syncState.finishedAt=new Date().toISOString();syncState.phase='complete';log('All enabled dealer syncs complete.');schedulePersist()}catch(e){syncState.errors.push(e.message);syncState.running=false;syncState.finishedAt=new Date().toISOString();syncState.phase='failed';log('FATAL '+e.message);schedulePersist()}}
